...
const Calendar = ({
  ...
  const getColor = useCallback((day: any) => {
    const d = dataByDay.find((d: any) => timeFormat('%b %d, %Y')(new Date(d.key)) === timeFormat('%b %d, %Y')(day));
    if (d) {
      if (d.value === 1) {
        return 'green';
      }
  ...
  const positionFromMonth = (month: any) => (timeFormat('%U')(new Date(`2019-${month}-01`)) * CELL_SIZE) + (timeFormat('%U')(new Date(`2019-${month}-01`)) * 3);
    ...
    <g transform={`translate(0, ${(130 * (maxYear - minYear + 1)) / 2})`}>
      ...
      {yearRange.map((year: any, i: any) => ( 
        <g transform={`translate(35, ${130 * i})`}>
          ...
          {timeDays(new Date(year, 0, 1), new Date(year + 1, 0, 1)).map((day: any) => <rect
